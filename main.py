...
# --- CARTERA SYNC (BITSO) ---
# Datos reales de tu última sesión
mxn_total = 116.1
usd_balance = 2.81
wallet = {{
    "ETH": 0.0017524,
...
if "precios" not in st.session_state:
    st.session_state.precios = [get_price() for _ in range(20)]
...
with col_left:
    ...
    # FIX: Configuración simplificada de Plotly para evitar ValueError
    fig = go.Figure()
...
with col_right:
    st.markdown('<div class="card" style="height:425px; text-align:left;">', unsafe_allow_html=True)
    ...
    if st.button("🚀 RE-ADAPTAR SISTEMA", use_container_width=True):
        st.rerun()
